...
def process_single_trader(
    ...
    """Process a single trader. Returns out_trader dict or None."""
    ...
    orphaned_by_parent: dict[str, list[input.Product]] = {}
    for var in potential_variants:
        if var.parent_class not in final_parents:
            orphaned_by_parent.setdefault(var.parent_class, []).append(var)

    for _parent_class, variants in orphaned_by_parent.items():
        if len(variants) >= 2:
            new_parent = variants[0]
            new_parent.parent_class = None
            final_parents[new_parent.className] = new_parent
            for var in variants[1:]:
                var.parent_class = new_parent.className
        else:
            var = variants[0]
...
def process_traders(
    ...
    for trader in traders_raw:
        result = process_single_trader(
            trader, products_all, prices_map,
            categories_template, loadouts_by_trader, output_dir,
        )
        if result is not None:
            output_traders.append(result)
...
def main(
    ...
    logger.info("Обработка торговцев…")
    ...
    logger.info("Сохранение конфигов…")
